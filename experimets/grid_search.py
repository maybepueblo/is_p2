...
# Intentamos añadir el directorio padre para encontrar los módulos
...
try:
    ...
    try:
        ...
        # Fallback final para estructura tipo 'code/'
...
def evaluar_configuracion(train_data, test_data, window_size, xgb_params):
    ...
    # 2. Inyección de Parámetros (Monkey Patching controlado)
    ...
    # 3. Entrenamiento
    # Nota: entrenar() ya se encarga de generar features usando self.WINDOW_SIZE
    # Capturamos stdout para que no ensucie la consola con los mensajes de 'Entrenando...'
    ...
    features_validas = [c for c in df_test_feat.columns if c not in cols_drop and 'tiempo' not in c]
...
def main():
    ...
    if not os.path.exists(RUTA_DATOS):
        # Fallback de ruta
        RUTA_DATOS = os.path.join(os.path.dirname(DIR_EXPERIMENTS), "data", "Dataset-CV.csv")
    ...
    print("-" * 95)
    print(f"{'WIN':<4} | {'DEPTH':<5} | {'EST':<4} | {'LR':<5} || {'ACCURACY':<8} | {'REC_BLK':<8} | {'F1_MACRO':<8}")
    print("-" * 95)
    ...
    print(f"   Scores:  Acc={mejor['acc']:.4f}, Rec_Block={mejor['recall_block']:.4f}, F1={mejor['f1']:.4f}")
